...
const cloudkitFinalizer string = "cloudkit.openshift.io/finalizer"
...
func (r *ClusterOrderReconciler) SetupWithManager(mgr ctrl.Manager) error {
	labelSelector := "cloudkit.openshift.io/clusterorder"
...
func (r *ClusterOrderReconciler) mapObjectToCluster(ctx context.Context, obj client.Object) []reconcile.Request {
	...
	labelVal, exists := obj.GetLabels()["cloudkit.openshift.io/clusterorder"]
	if !exists {
		return nil
	}
	...
	return []reconcile.Request{
		{
			NamespacedName: client.ObjectKey{
				...
				// FIXME: This should get set from an environment variable, which should be set
				// using a field selector in the deployment manifest.
				Namespace: cloudkitNamespaceName,
			},
		},
	}
}
